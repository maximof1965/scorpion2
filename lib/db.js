...
// Siguiente NUMERO unico: max global en INVENTARIO / VENDIDO / SALIDAS / RETIROS + 1, rango 100000-999999.
// Reintenta con huecos en cadena (numeroExisteGlobal) por datos historicos. La insercion
// hace otra revalidacion; si choca, ingreso.js reintenta.
const MIN_NUM_ART = 100000;
const MAX_NUM_ART = 999999;

async function maxNumeroGlobal() {
  const t = (tabla) =>
    supabase.from(tabla).select('NUMERO').order('NUMERO', { ascending: false }).limit(1);
  ...
  for (const n of nombres) {
    if (n.error && n.error.code && n.error.code !== 'PGRST116') {
      // eslint-disable-next-line no-console
      console.error('[db] maxNumeroGlobal', n.error);
    }
    const row = n.data && n.data[0] ? n.data[0] : null;
    if (row && row.NUMERO != null) vals.push(Number(row.NUMERO));
  }
  return vals.length ? Math.max(...vals) : 0;
}

async function siguienteNumeroInventario() {
  const max0 = await maxNumeroGlobal();
  let n = Math.max(MIN_NUM_ART, max0 + 1);
  for (let i = 0; i < 50; i += 1) {
    if (n > MAX_NUM_ART) {
      const err = new Error('RANGO_NUMEROS_AGOTADO');
      err.code = 'RANGO';
      throw err;
    }
    if (!(await numeroExisteGlobal(n))) return n;
    n += 1;
  }
  const e = new Error('NO SE PUDO OBTENER NUMERO UNICO');
  e.code = 'NUMERO_CARRERA';
  throw e;
}
